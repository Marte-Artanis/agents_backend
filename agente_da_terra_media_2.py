...
from cache_system import SimpleDiskCache
...
def groq_api_call(personagem, user_input, periodo_historico, fator_historico, idioma):
    ...
    formatted_prompt_str = "".join([msg.content for msg in formatted_prompt])  # Converter para string
    print("Formatted Prompt String:", formatted_prompt_str)

    # Verificar se o cache está funcionando
    cache_key = hashlib.md5((user_input + formatted_prompt_str).encode()).hexdigest()  # Incluir a entrada do usuário na chave do cache
    print("Cache Key:", cache_key)
    
    # Consultar o cache antes de fazer a chamada ao modelo
    cached_response = cache.lookup(cache_key, formatted_prompt_str)
    if cached_response:
        print("Cache hit")
        resposta = cached_response.strip()
    else:
        print("Cache miss")
        response = chat.invoke(formatted_prompt)
        resposta = response.content.strip()
        print("Storing in cache:", cache_key, resposta)
        cache.update(cache_key, resposta, formatted_prompt_str)  # Armazenar a resposta no cache
